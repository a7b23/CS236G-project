    ...
    def __init__(self, config, weight_init):
        ...
        with tf.device('{}:*'.format(config.device)):
            with tf.name_scope('encoder'):
                ...
                self.res_dense0 = ops.resblock_dense_no_sn(units=512, weight_init=self.weight_init)
                self.res_dense1 = ops.resblock_dense_no_sn(units=512, weight_init=self.weight_init)
                self.dense0 = Dense(units=config.num_cont_noise*2, kernel_initializer=self.weight_init)
                self.flatten = Flatten()
